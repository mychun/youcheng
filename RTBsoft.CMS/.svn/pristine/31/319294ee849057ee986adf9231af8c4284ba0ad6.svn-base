...
namespace RTBcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void user_login(HttpContext context)
        {
            ...
            {
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "RTBcms", model.user_name, 43200);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "RTBcms", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "RTBcms", model.user_name);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "RTBcms", model.password);
            }
        ...
        private void user_register(HttpContext context)
        {
            ...
            string password = "123456";//DTRequest.GetFormString("txtPassword").Trim();
            string email = Utils.ToHtml(DTRequest.GetFormString("txtEmail").Trim());
            string mobile = Utils.ToHtml(DTRequest.GetFormString("txtMobile").Trim());
            string qq = Utils.ToHtml(DTRequest.GetFormString("txtQQ").Trim());
            string address = Utils.ToHtml(DTRequest.GetFormString("txtAddress").Trim());
            string userip = DTRequest.GetIP();
            ...
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，手机号码不能为空！\"}");
                return;
            }
            //如果开启邮箱注册则要验证邮箱
            ...
            {
                context.Response.Write("{\"status\":1, \"msg\":\"信息提交成功，请等待审核通过！\", \"url\":\""
