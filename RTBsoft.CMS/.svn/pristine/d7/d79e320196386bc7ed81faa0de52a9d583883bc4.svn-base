            ...
            if (xnList.Count > 0)
            {
                foreach (XmlElement xe in xnList)
                {
                    if (xe.NodeType != XmlNodeType.Comment && xe.Name.ToLower() == "nav")
                    {
                        ...
                        if (xe.Attributes["parentname"] != null)
                        {
                            parentName = xe.Attributes["parentname"].Value;
                        }
                        else {
                            parentName = temp;
                        }
                        int nav_id = new navigation(databaseprefix).Add(parentName, nav_name, nav_title, link_url, 110, 0, action_type); //写入数据库
                        if (nav_id < 1)
                        {
                            return false;
                        }
