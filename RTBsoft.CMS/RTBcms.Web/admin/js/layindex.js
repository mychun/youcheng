...
function initMenuTree(islink) {
	...
	navGroupObj.each(function (i) {
	    ...
	    //绑定h2主标题点击
	    $(this).children(".list-wrap").children("h2").click(function () {
	        var thisSiblings = $(this).siblings();
            //显示隐藏子菜单
	        if (thisSiblings.css("display") == "none") {

	            $(this).removeClass("close").addClass("open");
	            thisSiblings.slideDown(function () {

	                //如果子菜单只有一个，则直接显示子菜单内容
                    /**
	                if (thisSiblings.children("li").length == 1) {
	                    thisSiblings.children("li").children("a").children(".expandable").removeClass("close").addClass("open");
	                    thisSiblings.children("li").children("a").siblings("ul").slideDown();
	                }**/
	            });
	        } else {

	            $(this).removeClass("open").addClass("close");
	            thisSiblings.slideUp();
	            thisSiblings.find("ul").slideUp();

                //隐藏子菜单同时，隐藏子菜单的菜单
	            var thisSiblingsA = thisSiblings.find("a");
	            thisSiblingsA.each(function () {
