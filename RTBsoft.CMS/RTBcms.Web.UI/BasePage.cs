...
namespace RTBcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        public string linkurl(string _key, params object[] _params)
        {
            ...
            if (model.url_rewrite_items.Count == 0)
            {
                ...
                else
                {
                    if (_params.Length > 0)
                    {
                        return linkStartString + model.page.Replace("aspx", config.staticextension) + string.Format("{0}", _params);
                    }
                    else
                    {
                        return linkStartString + model.page.Replace("aspx", config.staticextension);
                    }
                }
            }
            //否则检查该URL配置节点下的子节点
            foreach (Model.url_rewrite_item item in model.url_rewrite_items)
            {
                ...
                if (IsUrlMatch(item, _params))
                {
                    ...
                    else
                    {
                        ...
                        return linkStartString + model.page.Replace("aspx", config.staticextension) + queryString;
                    }
                }
            }
        ...
        public bool IsUserLogin()
        {
            ...
            {
                //检查Cookies
                string username = Utils.GetCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "RTBcms");
                string password = Utils.GetCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "RTBcms");
